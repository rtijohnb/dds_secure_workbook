...
openssl genrsa -out ./private/permissionscakey.pem

echo "  Generating Certificate Signing Request: ca.csr"
openssl req -new -key ./private/permissionscakey.pem -out ca.csr -config openssl.cnf

echo "  Generating and self-signing the Permissions CA certificate"
openssl x509 -req -days 3650 -in ca.csr -signkey ./private/permissionscakey.pem -out permissionscacert.pem
